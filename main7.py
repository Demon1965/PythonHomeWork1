...
""" def check_rythm(text):
    ...
    puh_unic= set()
    for i in puh_list:
        puh_unic.add(len(list(filter(lambda z:True if z in alph else False, i))))
        #print(puh_unic)
    if len(puh_unic)>1:
        return print("Пам парам")            
    return print("Парам пам-пам")

check_rythm(text=str(input("Please, insert Vinni Puh'text!:  "))) """
...
""" def print_operation_table(operation, rows=6, columns=6):
    print(*[i+1 for i in range(columns)])
    for i in range(2, rows+1):
        print(*[i if j == 1 else operation(j, i)
